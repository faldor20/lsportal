...
*)
module Eio_Flow_Channel= struct
  open Eio
  ...
  let read_line ic =
    ...
    | str ->
      Some str
    | exception e ->
      traceln "exception reading line%s" (Base.Exn.to_string e);
      None
  ;;

  let read_exactly ic len =
    match Eio.Buf_read.take len ic with
    | str ->
      Some str
    | exception exn ->
      traceln "exception reading exactly%s" (Base.Exn.to_string exn);
      None
  ;;

  let write oc strings = strings |> List.iter ~f:(Eio.Buf_write.string oc)
end

module Io = Packet_io.Make (Eio_Flow_Channel)
...
let send ((_, oc, m) : t) packets =
  (* traceln
     ...
     x |> Jsonrpc.Packet.yojson_of_t |> Jsonrpc_runner.Json.to_pretty_string )
     |> String.concat ) ;*)
  Eio.Mutex.use_rw ~protect:true m (fun () ->
    Buf_write.with_flow oc (fun writer -> packets |> List.iter ~f:(Io.write writer)))
;;
...
let make ic oc : t =
  ...
  ic |> Buf_read.of_flow ~max_size:(1000 * 1000), (oc :> sink), Eio.Mutex.create ()
;;
...
let close ((ic, oc, _) : t) what =
  (match what with
   | `Write ->
     Flow.close oc
   | `Read ->
     (*not sure if i need a proper way to close this*) ());
  ()
;;
